...
export const Edit = ( props ) => {
  ...
  //{ iconType, iconValue, iconPath, iconColor, iconSize, alignment } = attributes,
  { blockClassName, icon } = attributes;


  // Apply block id if not already set
  useEffect(() => {
    if ( ! blockClassName ) setAttributes( { blockClassName: `icon-block-${ clientId }` } );
  }, [ clientId, blockClassName, setAttributes ] );


  const 

  // Use the saved blockClassName (or fall back to clientId for the immediate first render)
  uniqueClassName = blockClassName || `icons-block-${clientId}`,

  // block props
  blockProps = useBlockProps({
    onClick: () => console.log("Attributes:", attributes), 
    className: `${uniqueClassName } ${icon?.link?.hasLink ? "__has_link" : ""}`
  }),

  handleSelect = (data) => {
    console.log("Selected Label:", data.label);
    setAttributes({
      icon: {
        ...icon,
        type: data.type,
        path: typeof data.path === "string" ? data.path : "", // Only for FA
        value: data.value,
        label: data.label,
      },
    });
  ...
  template = () => (<> {/* Selected State: Just show the icon, no buttons blocking view */}
    {icon?.type === "fa" &&
  ...
  return (
    <>
      <Sidebar {...props} handleSelect={handleSelect} />
      <Toolbar {...props} handleSelect={handleSelect} />
      <style type="text/css">
        {(() => `
        .${uniqueClassName} {
          width: auto;
      ...
      <div {...blockProps}>
        {icon?.type === "" ? (
          /* Empty State: Show a Placeholder */
          <Placeholder
            icon={starFilled}
            label="Icon Block"
            instructions="Select an icon from the library or upload an SVG."
          >
            <IconPicker
